...
ได้ไฟล์ `.pgm` กับ `.yaml` คู่กัน **ใช้ตัวดิบนี้กับ AMCL ได้เลย**

### อย่าแต่งแมพเพื่อช่วย AMCL

เอกสารเก่าและแผน O3 บอกให้แต่งแมพเพื่อให้ AMCL จับได้ดีขึ้น **วัดแล้วว่าตรงข้าม**
ผลจาก `LIVOX_AMCL_TUNING_2026-09-03.md` แมพเดียวกัน bag เดียวกัน อย่างละสองรอบ

```text
                  แมพดิบ    แต่งรอบเดียว   แต่งสองรอบ
ค่าคลาดเฉลี่ย       0.271       0.365         0.594 m
มุมคลาดเฉลี่ย        7.10        9.93         16.35 deg
```

แย่ลงตามปริมาณที่แต่ง และไม่เสถียรด้วย — ตัวที่แต่งสองรอบให้ 0.698 กับ 0.491
ในสองรอบที่เหมือนกันทุกอย่าง

เหตุผล: การแต่งคือการวาดกำแพงที่ไม่มีอยู่จริง แต่สแกนจริงยังยิงทะลุตรงนั้นได้
ทุกครั้งที่หุ่นหันไปทางนั้น AMCL จะได้ลำแสงที่บอกว่า "ว่าง" ทับแมพที่บอกว่า "กำแพง"

**ประโยชน์ของการแต่งแมพอยู่ที่ Nav2 ไม่ใช่ AMCL** — ปิดรูรั่วเพื่อไม่ให้ planner
วางเส้นทางทะลุกำแพงออกไปนอกอาคาร ถ้าจำเป็นต้องทำ ให้แยกสองไฟล์

```bash
ros2 launch go2_control livox_ground.launch.py \
  amcl_params_file:=amcl_livox.yaml \
  map:=แมพดิบ.yaml
# แล้วชี้ global costmap ของ Nav2 ไปที่แมพที่แต่งแล้วแยกต่างหาก
```

ถ้าแต่งจริงแล้วเซฟเป็นชื่อใหม่ **ต้องแก้บรรทัด `image:` ใน `.yaml` ให้ชี้ไฟล์ใหม่**
ไม่งั้น map_server จะโหลดตัวเก่าโดยไม่เตือน หรือโหลดไม่ขึ้นเลยถ้าไฟล์ไม่มีจริง
...
```text
ยังไม่เคยยืนยันว่า --mode api ทำให้ Go2W เดินจริง   ← ความเสี่ยงหลักของทริปนี้
ตัวเชื่อม UDP ยังไม่อยู่ใน launch file ต้องรันมือสองเทอร์มินัล
ยังไม่เคยวัดว่าบอร์ดมี CPU พอไหม  ใช้ go2cpu วัดที่ไซต์
เส้นทางถ่ายรูปและอัปโหลดกลับเว็บยังไม่ได้เขียน
AprilTag ยังไม่เคยทดสอบที่ไซต์จริง
send_mission กับ record_waypoint ยังไม่เคยรันกับสแตกที่เดินได้จริง
```

**ค่าคลาดเคลื่อนที่วัดได้จาก replay** คือ 0.258 ม. เฉลี่ย สูงสุด 0.494 ม.
ตลอดการเดิน 33.7 ม. ยังไม่มีตัวเลขจากหุ่นจริง

```text
ที่โล่ง เป้าเดียว ทางกว้าง     พอ
ประตูกว้าง 0.9 ม.              เฉียดฉิว
หยุดถ่ายรูปให้ตรงจุด            ไม่พอ ต้องใช้ AprilTag
```
...
**อย่า** source `setup_robot_env.sh` หรือ `setup_ground_env.sh` สองตัวนั้นตั้ง
CycloneDDS ผูกกับการ์ดเครือข่าย ซึ่งเป็นเรื่องของการแยกสองเครื่อง
งานที่บ้านอยู่เครื่องเดียวและใช้ Fast DDS บน loopback

`go2local` จะพิมพ์ค่า load average ให้ดูด้วย **รอให้ต่ำกว่า 3.0 ก่อนเริ่ม**
FAST-LIO ไม่ได้แค่ช้าลงเมื่อ CPU ไม่พอ แต่ตำแหน่งจะระเบิดเป็นหลักหมื่นเมตร
โดยที่ทุกโปรเซสยังรันอยู่ปกติ
...
**ใช้ตรวจตรรกะของ send_mission และ record_waypoint** ซึ่ง bag ทดสอบให้ไม่ได้

เหตุผล: เล่น bag แล้วหุ่นไม่ขยับ ไม่ว่า Nav2 จะสั่งอะไร สุดท้าย Nav2 จะ abort
เสมอ จึงไม่มีทางเห็นว่า goal สำเร็จจริงไหม, เดินจบจุดที่ 1 แล้วไปจุดที่ 2 ถูกไหม,
capture pause ทำงานไหม, Ctrl-C แล้วหุ่นหยุดจริงไหม

ใน Gazebo หุ่นขยับตามคำสั่งจริง จึงปิดวงจรได้ครบ

```bash
ros2 launch go2_control sim.launch.py
ros2 run go2_control send_mission --goal 1.0 0.0 0.0
ros2 run go2_control record_waypoint --file missions/sim.json
ros2 run go2_control send_mission --file missions/sim.json
```

**ข้อจำกัด** TurtleBot3 มี LiDAR 2D จริง สแกนสวยกว่า Mid-360 ที่ฝานมามาก
AMCL ใน sim จะลู่เข้าสวยงามซึ่งไม่ได้แปลว่าของจริงจะเป็นแบบนั้น และหุ่นเป็น
TurtleBot3 ขนาดกับ footprint คนละเรื่องกับ Go2W

พูดง่าย ๆ **sim พิสูจน์ว่าโค้ดถูก ไม่ได้พิสูจน์ว่าจะใช้ได้หน้างาน**
