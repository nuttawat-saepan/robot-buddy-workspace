...
class MultiGoal(Node):
    def __init__(self):
        ...
        self.enable_mqtt = self.declare_parameter('enable_mqtt', False).value
        self.cmd_vel_enabled = self.declare_parameter('cmd_vel_enabled', False).value
        self.robot_ack = self.declare_parameter('robot_ack', '').value
        self.mqtt_broker = self.declare_parameter('mqtt_broker', '192.168.68.62').value
        self.mqtt_port = self.declare_parameter('mqtt_port', 1883).value
        self.img_dir = self.declare_parameter(
            'image_dir',
            os.path.expanduser('~/UnitreeRos/unitree_ros2/images'),
        ).value

        self.last_battery_percent = 100.0 if self.sim_mode else None
        self.sim_battery_drain_rate = 0.5  # percent per publish interval
        if self.cmd_vel_enabled and self.robot_ack != REQUIRED_ROBOT_ACK:
            self.get_logger().error(
                'cmd_vel_enabled requested but robot_ack is missing or invalid; '
                'disabling /cmd_vel publishing'
            )
            self.cmd_vel_enabled = False
        self.get_logger().warn(
            f'cmd_vel publishing enabled: {self.cmd_vel_enabled}'
        )
        self.get_logger().warn(f'MQTT enabled: {self.enable_mqtt}')
        ...
        if self.enable_mqtt:
            ...
            while True:
                try:
                    self.mqtt.connect(self.mqtt_broker, int(self.mqtt_port), 60)
                    break
            ...
            self.get_logger().warn("MQTT disabled by parameter")
        ...
        if self.enable_mqtt:
            ...
            self.map_publish_timer = self.create_timer(10.0, self.map_sync_loop)
    ...
    def stop_robot(self):
        if not self.cmd_vel_enabled:
            self.get_logger().warn("cmd_vel disabled; not publishing stop Twist")
            return
    ...
    def publish_cmd_vel(self, vx, wz, vy=0.0):
        try:
            if not self.cmd_vel_enabled:
                self.get_logger().warn("cmd_vel disabled; dropping velocity command")
                return
            self.get_logger().info(f"🎮 Parsed cmd_vel: vx={vx}, vy={vy}, wz={wz}")
            twist = Twist()
            twist.linear.x = vx
