...
class AprilLocalizer(Node):
    def __init__(self):
        ...
        self.cmd_vel_enabled = bool(
            self.declare_parameter("cmd_vel_enabled", False).value
        )
        self.robot_ack = self.declare_parameter("robot_ack", "").value
        self.cmd_vel_armed = (
            self.cmd_vel_enabled and self.robot_ack == REQUIRED_ROBOT_ACK
        )
        if self.cmd_vel_enabled and not self.cmd_vel_armed:
            self.get_logger().error(
                "AprilTag cmd_vel scan requested but robot_ack is missing or invalid; "
                "scan motion is disabled"
            )
    ...
    def request_cb(self, msg):
        ...
        if scan and not self.cmd_vel_armed:
            self.get_logger().warn(
                "Localization request asked for scan motion, but cmd_vel is not armed; "
                "continuing without scan motion"
            )
            scan = False
        timeout = float(
            localize.get(
