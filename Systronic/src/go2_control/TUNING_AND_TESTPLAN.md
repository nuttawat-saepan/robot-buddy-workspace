...
## 11. เทียบ 0.05 กับ 0.25 อีกครั้งบน bag ที่เดินทั้งห้อง

`02_loopfix` เหมาะกว่า bag หน้างานสำหรับคำถามนี้ — เดินวน 180 วินาที 1794 สแกน
ผ่านหลายมุมของห้อง และ**ไม่มี `/tf` อัดมาด้วย** จึงไม่มีปัญหาปนเปื้อนตามหัวข้อ 10

```text
                  ลำแสงต่อสแกน   ระยะใกล้สุดเฉลี่ย
min_height 0.25        615            1.01 m
min_height 0.05        635            0.99 m

เพิ่มขึ้น +20 ลำแสงต่อสแกน (3.3%)
สแกนที่ 0.05 เห็นของใกล้กว่าเกิน 5 ซม.: 188 จาก 1794 (10%)  ← สิบเท่าของ bag หน้างาน
ใกล้กว่าเฉลี่ย 0.14 m สูงสุด 0.40 m
```

ดูเผิน ๆ เหมือนเจอของที่ 0.25 มองไม่เห็นจริง แต่พอดูว่าของนั้นอยู่ไกลแค่ไหน

```text
0.5-1.0 m   152 ครั้ง  (81%)
1.0-2.0 m    35 ครั้ง  (19%)
2.0-4.0 m     1 ครั้ง
median 0.72 m
```

ใกล้เกินกว่าจะเป็นของในห้อง จึงวัดต่อว่ามันอยู่ที่มุมไหนในเฟรมของหุ่น
ถ้าเป็นของในโลก มุมจะกระจายเมื่อหุ่นหมุนและเดิน ถ้าติดอยู่กับหุ่น มุมจะกระจุก

```text
มุมของลำแสงที่เห็นเพิ่ม ใกล้กว่า 1.2 m  (0° = หน้าหุ่น)
  220 deg  ###
  230 deg  ###############
  240 deg  ################
  250 deg  ###########
...
**กระจุกอยู่ในเสี้ยวเดียวราว 100 องศา ด้านหลังเยื้องขวา** ตลอดทั้ง bag
แปลว่าสิ่งนั้น**เคลื่อนที่ไปกับหุ่น** ไม่ใช่ของที่วางอยู่ในห้อง จะเป็นคนที่เดิน
ตามอยู่ สายที่ลาก หรือของที่ติดอยู่บนตัวหุ่น ข้อมูลชุดนี้แยกไม่ได้ว่าอันไหน
แต่แยกได้ว่ามันไม่ใช่สิ่งกีดขวางในห้อง

### ข้อสรุป

**การลด min_height เป็น 0.05 ตามข้อมูลที่มีอยู่ ให้ผลเสียมากกว่าผลดี**
สิ่งที่ได้เพิ่มส่วนใหญ่คือวัตถุที่ติดไปกับหุ่น ซึ่งจะกลายเป็นสิ่งกีดขวางถาวร
ในตำแหน่งเดิมของ local costmap ตลอดเวลา Nav2 จะพยายามหลบของที่หลบไม่ได้

และยังไม่ตอบคำถามความปลอดภัยอยู่ดี เพราะทั้งสอง bag ไม่มีสิ่งกีดขวางเตี้ยจริง
วางอยู่เลย **การทดสอบที่ตอบได้มีทางเดียว** — วางกล่องสูงราว 15 ซม. ห่างหุ่น
สองเมตร อัดสัก 30 วินาที แล้วรันเทียบแบบเดียวกันนี้ ถ้ากล่องโผล่ที่ 0.05
และไม่โผล่ที่ 0.25 นั่นคือคำตอบ และตอนนั้นค่อยตัดสินใจว่าจะทำ `/scan_low`
แยกสำหรับ costmap หรือไม่
