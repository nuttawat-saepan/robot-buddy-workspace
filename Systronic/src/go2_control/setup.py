...
setup(
    ...
    data_files=[
        ...
        # Every launch file, by glob. They used to be listed one by one,
        # and a new one that was never added to the list simply did not
        # install - `ros2 launch` then says the file is not in the share
        # directory, which reads as a missing file rather than a missing
        # line in setup.py. occupancy_grid_points had the same problem as
        # an entry point and went unnoticed for months.
